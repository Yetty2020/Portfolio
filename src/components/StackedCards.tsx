            ...
            <div ref={stackedCards} className='stacked-cards w-full h-[100vh] relative mx-auto md:w-[75%] flex flex-col xl:w-[clamp(85%, 4vw, 90%)] '>

              {
                ProjectItems.map((item, index) => (
                  <div key={item.id} className='absolute inset-0 m-auto card w-full   lg:h-[75vh] flex flex-col-reverse column-reverse lg:flex-row rounded-md shadow-md  border border-gray-500 p-6 overflow-hidden bg-black' style={{ zIndex: index + 1 }}>
                    <div className='card-content w-full lg:w-1/2 lg:h-[40%] lg:h-full flex flex-col gap-5  justify-content bg-black'>
                      <h4 className='self-start text-5xl hidden lg:flex'>{item.num}</h4>
                      <p>(About The Project)</p>
                      <p className='lg:w-[90%]'>{item.about}</p>
                    </div>
                    <div className='img-wrapper lg:w-1/2 lg:h-[80%] lg:h-full w-full lg:w-1/2 lg:flex lg:flex-col lg:gap-6 object-contain'>
                      <h4 className='self-start text-xl lg:hidden mb-2 '>{item.num}</h4>
